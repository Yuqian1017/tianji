...
const REFERENCE_PATH = 'database/tcm/skill-v3/references/45-经典-金匮要略杂病方证条文.md';
const ADJUDICATION_PATH = 'database/tcm/adjudications/tcm-classic-ref45-quote-adjudications.json';
const PUBLIC_WITNESS_PATH = 'database/sources/ctext/tcm-jingui-public-witnesses-2026-07-18.json';
const NIHAISHA_SOURCE_PATH = 'database/sources/external/nihaisha-nishi-tcm-2026-07-18.json';
const OUTPUT_PATH = 'database/tcm/normalized/tcm-classic-ref45-quote-provenance.json';
...
const EXPECTED_SOURCE_OVERRIDES = new Map([
  ['tcm.classic.ref45.quote.L0064Q1', 'huangdi-neijing-suwen-local-text'],
]);
...
function expectedSourceIds(id) {
  return [EXPECTED_SOURCE_OVERRIDES.get(id) ?? 'jingui-yaolue-local-text'];
}
...
for (const adjudication of adjudicationCore.records) {
  if (adjudications.has(adjudication.id)) {
    throw new Error(`Duplicate ref45 quote adjudication: ${adjudication.id}`);
  }
...
for (const [lineIndex, line] of referenceText.split('\n').entries()) {
  ...
  for (const match of line.matchAll(/“([^”\n]+)”|"([^"\n]+)"/g)) {
    ...
    const id = `tcm.classic.ref45.quote.L${String(referenceLine).padStart(4, '0')}Q${quoteIndex}`;
    ...
    const externalClassicalAllusion = expectedSources[0] !== 'jingui-yaolue-local-text';
...
const sourceRefs = Object.fromEntries([
  ['ref45', REFERENCE_PATH, referenceText],
  ...sources.map(source => [source.id, source.path, source.text]),
  ['ref45-manual-adjudications', ADJUDICATION_PATH, adjudicationText],
  ['ref45-public-transcription-witnesses', PUBLIC_WITNESS_PATH, publicWitnessText],
...
const output = {
  ...
  scope: 'reference_45_jingui_explicit_quotes',
  ...
  boundaries: [
    ...
    'Manual form adjudication records abridgement, stitching, editorial prose, OCR errors, and transcription variants without accepting a traditional interpretation or modern medical claim.',
    ...
    'nihaisha-nishi-tcm page cards are secondary locators only; where a page card repeats 火逆 or modernizes 㽲痛, it cannot override the independent classical witnesses.',
    'Duplicate matches in Jingui Yaolue and Shanghanlun are retained as parallel textual witnesses, with Jingui Yaolue selected for this reference.',
