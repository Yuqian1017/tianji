...
const OUTPUT_PATH = path.join(ROOT, 'database/tcm/sources/chp2025-tcm-pregnancy-compatibility-index.json');
...
const tempDir = await mkdtemp(path.join(os.tmpdir(), 'tianji-chp2025-tcm-safety-'));
...
const items = names.map(name => ({
  requestedName: name,
  normalizedCatalogTitle: normalizeCatalogTitle(name),
  titleIn2025Catalog: catalogTitles.has(normalizeCatalogTitle(name)),
}));
...
const result = {
  ...
  scope: 'tcm_pregnancy_compatibility_and_aristolochic_candidates',
  ...
  counts: {
    requestedNames: items.length,
    currentMonographTitles: items.filter(item => item.titleIn2025Catalog).length,
    currentTitlesMissing: items.filter(item => !item.titleIn2025Catalog).length,
  },
  items,
};
