...
import { ZIWU_LIUZHU, WUXING_COLORS } from './data.js';
...
function MeridianCard({ entry }) {
  ...
  return (
    <div className="bg-[var(--color-bg-card)] card-blur border border-[var(--color-gold-border)] rounded-xl p-4 space-y-3">
      ...
      <div className="border border-[var(--color-surface-border)] bg-[var(--color-surface-dim)] rounded-lg p-2 text-xs text-[var(--color-text-dim)]">
        此处只展示常见传统对应关系；不代表器官功能高峰，也不用于症状归因或治疗时机判断。
      </div>
    </div>
  );
}
        ...
        <div className="divide-y divide-[var(--color-surface-border)]">
          {ZIWU_LIUZHU.map((entry, i) => (
            <div key={i} className={`flex items-center gap-2 px-3 py-1.5 text-xs ${i === activeIndex ? 'bg-[var(--color-gold-bg)]' : ''}`}>
              ...
              <span className="flex-1 text-[var(--color-text)] truncate">{entry.meridian}</span>
