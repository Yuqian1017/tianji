...
- 日柱不再依赖浏览器本地时区的 `Date`。
- 一月出生的大运能找到真正相邻的前/后节，并保存精确起运年月日。
...
- 输出稳定的年、月、日、时四柱。
- 输出当前月令对应的节名。
- 从 `EightChar.getYun()` 输出大运方向、精确起运间隔、交运公历日期和八步大运。
...
`isApprox` 对新历法核心固定为 `false`。`startAge` 保持当前整数展示语义，按精确年月日折算后四舍五入且最小为 1；精确值保存在 `dayunStart`，避免再次丢失。
...
3. `2025-01-01` 男/女起运整数分别为 1/9，精确值分别约为 1年3月20日/8年6月10日。
