...
7. 现有资产优先复用。当前工具、账户、历史记录、AI Provider 和本地算法保留，除非验证证明必须替换。
...
实战工具承担统一产品的实践与验证环节，当前代码可继续作为这一层的主体。
...
保留：前端框架、模块接口、排盘引擎、账户/历史、AI 调用层和已验证交互。
...
- 通过后解锁筑基入口和一个与 64 卦/六爻相关的预览或实战引导。
- 错题回到具体知识条目，并给出来源化解释。
...
完成条件：主 PRD、Coverage Matrix、Source Manifest 一致；主要来源已登记；用户确认产品关系和首个垂直切片。
